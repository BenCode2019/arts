 ...
 *
 * Created by mengwei on 2019/8/16.
 */
public class Permutations2 {

    private void findPermuteUnique(List<List<Integer>> res,boolean[] used,int[] nums, int depth, Stack<Integer> stack) {
        ...
        for (int i = 0; i < nums.length; i++) {
            if (!used[i]) {
                ...
                //
                // 注意： !used[i - 1] == true表示同层的上一个相等的值没有被使用，
                // 证明已经回溯回来所以可以肯定是重复的。这种情况跳过即可
                if (i > 0 && nums[i] == nums[i - 1] && !used[i - 1]) {
                    continue;
                }
                used[i] = true;
                stack.add(nums[i]);
                findPermuteUnique(res,used,nums, depth + 1, stack);
                stack.pop();
                used[i] = false;
            }
        }
    ...
    public List<List<Integer>> permuteUnique(int[] nums) {
        ...
        findPermuteUnique(res ,used,nums, 0, new Stack<Integer>());
        return res;
    }
